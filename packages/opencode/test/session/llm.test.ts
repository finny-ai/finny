...
describe("session.llm.stream", () => {
  ...
  it.instance(
    "keeps synthetic invalid tool active for repaired tool-call failures",
    () =>
      Effect.gen(function* () {
        const fixture = loadFixture(alibabaQwenFixture.providerID, alibabaQwenFixture.modelID)
        const request = waitRequest(
        ...
        const sessionID = SessionID.make("session-test-invalid-tool")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
          permission: [{ permission: "invalid", pattern: "*", action: "deny" }],
        } satisfies Agent.Info

        const user = {
          id: MessageID.make("msg_user-invalid-tool"),
          sessionID,
          role: "user",
          time: { created: Date.now() },
          agent: agent.name,
        ...
        const capture = yield* Effect.promise(() => request)
        const tools = capture.body.tools as Array<{ function?: { name?: string } }> | undefined
        expect(tools?.some((item) => item.function?.name === "invalid")).toBe(true)
      }),
    {
      config: () => ({
        enabled_providers: [alibabaQwenFixture.providerID],
