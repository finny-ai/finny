...
import { Git as CoreGit } from "@opencode-ai/core/git"
...
function closedLayer<R, E>(layer: Layer.Layer<R, E, any>): Layer.Layer<R, E> {
  return layer as Layer.Layer<R, E>
}
...
function projectLayerWithFailure(failArg: string) {
  const gitLayer = CoreGit.layer.pipe(
  ...
  return Project.layer.pipe(
    ...
    Layer.provide(ProjectV2.layer.pipe(Layer.provide(gitLayer))),
...
const failureIt = (failArg: string) =>
  testEffect(closedLayer(Layer.mergeAll(projectLayerWithFailure(failArg), CrossSpawnSpawner.defaultLayer)))
