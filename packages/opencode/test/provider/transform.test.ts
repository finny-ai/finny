...
describe("ProviderTransform.options - gpt-5 textVerbosity", () => {
  ...
  const prepareWithPriorToolCall = (input: { model?: any; provider?: any } = {}) => {
    const model = input.model ?? createGpt5Model("gpt-5.2")
    return Effect.runPromise(
      LLMRequestPrep.prepare({
        ...
        tools: {},
        provider: input.provider ?? ({ id: model.providerID, options: {} } as any),
        auth: undefined,
        plugin: {
  ...
  test("LiteLLM proxy requests replaying tool calls include noop compatibility tool", async () => {
    const result = await prepareWithPriorToolCall({
      provider: { id: "anthropic", options: { litellmProxy: true } },
    })
    expect(result.tools._noop).toBeDefined()
  })

  test("LiteLLM model/provider ids replaying tool calls include noop compatibility tool", async () => {
    const result = await prepareWithPriorToolCall({
      model: {
        ...createGpt5Model("gpt-5.2"),
        id: "litellm/claude-sonnet-4",
        providerID: "custom-litellm",
      },
      provider: { id: "custom-litellm", options: {} },
    })
    expect(result.tools._noop).toBeDefined()
  })
...
describe("ProviderTransform.schema - openai supported schema subset", () => {
  ...
  const unsupportedKeywordSchema = {
  ...
  const sanitizedUnsupportedKeywordSchema = {
    type: "object",
    properties: {
      query: {
        type: "string",
  ...
  test("removes unsupported JSON Schema keywords recursively", () => {
    expect(ProviderTransform.schema(openaiModel, unsupportedKeywordSchema)).toEqual(sanitizedUnsupportedKeywordSchema)
  })
...
describe("ProviderTransform.schema - moonshot $ref siblings", () => {
  ...
  test("moves parent type into anyOf items for Kimi coding schemas", () => {
    const result = ProviderTransform.schema(moonshotModel, {
      type: "object",
      properties: {
    ...
    expect(result.anyOf).toEqual([
      { type: "object", required: ["description", "prompt", "subagent_type"] },
