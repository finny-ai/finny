...
describe("tool.edit", () => {
  ...
  describe("researcher workspace boundaries", () => {
    it.live("allows bound researcher edits inside workspace news", () =>
      provideTmpdirInstance((dir) =>
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(yield* loadRaw(filepath)).toBe("# Regime\n")
          } finally {
    ...
    it.live("blocks bound researcher edits outside workspace news", () =>
      provideTmpdirInstance((dir) =>
        Effect.gen(function* () {
          const prev = process.env.XDG_DATA_HOME
          process.env.XDG_DATA_HOME = dir
          try {
            ...
            if (Exit.isFailure(exit)) {
              ...
              expect(error instanceof Error ? error.message : String(error)).toContain("Researcher edit blocked")
            }
    ...
    it.live("blocks unbound researcher edits to repo-local news", () =>
    ...
    it.live("blocks unbound researcher edits to ad hoc repo news folders", () =>
      provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            yield* Effect.promise(() => clearSessionWorkspace(researcherCtx.sessionID).catch(() => {}))
            const filepath = path.join(dir, "packages/opencode/aapl-news-headlines/2026-06-12.md")
