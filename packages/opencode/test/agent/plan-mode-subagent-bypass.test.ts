...
it.instance("subagent permissions take precedence over parent agent restrictions", () =>
  Effect.gen(function* () {
    const buildAgent = yield* Agent.use.get("build")
    const generalAgent = yield* Agent.use.get("general")

    expect(buildAgent).toBeDefined()
    expect(generalAgent).toBeDefined()
    // Sanity: the parent Build agent itself blocks edit. (Note: `write` and
    // `apply_patch` route through the `edit` permission at the runtime
    // tool layer — see Permission.disabled / EDIT_TOOLS.)
    expect(Permission.evaluate("edit", "/some/file.ts", buildAgent!.permission).action).toBe("deny")
