...
test("renders mandatory evidence batch at task start with child labels", async () => {
  const out = await setup()
  ...
  try {
    await out.scrollback.append(
      toolCommit({
        ...
        state: {
          ...
          input: {
            tasks: [
              { subagent_type: "data_extractor", description: "Extract BTC 15min data" },
              { subagent_type: "news_agent", description: "Extract BTC news context" },
            ],
          },
        },
      }),
    ...
    try {
      const output = render(commits)
      expect(output).toContain("Mandatory Evidence Batch")
      expect(output).toContain("Data Extractor")
      expect(output).toContain("Extract BTC 15min data")
      expect(output).toContain("News Agent")
      expect(output).toContain("Extract BTC news context")
    } finally {
      destroy(commits)
    }
  } finally {
