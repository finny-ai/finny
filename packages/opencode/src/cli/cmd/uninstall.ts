...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall finny and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g finny-ai",
      pnpm: "pnpm uninstall -g finny-ai",
      bun: "bun remove -g finny-ai",
      yarn: "yarn global remove finny-ai",
      brew: "brew uninstall finny",
      choco: "choco uninstall finny",
      scoop: "scoop uninstall finny",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall finny -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using Finny!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# finny") || content.includes(".finny/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".finny/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".finny/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".finny"))
    ) {
      continue
    }
