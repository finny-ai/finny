...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run finny",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for finny session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found finny session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`finny import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting finny...")
        UI.println()

        // Launch finny TUI with session ID if available
        const { spawn } = await import("child_process")
        const finnyArgs = sessionId ? ["-s", sessionId] : []
        const finnyProcess = spawn("finny", finnyArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          finnyProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`finny exited with code ${code}`))
          })
          finnyProcess.on("error", reject)
        })
      },
    })
  },
