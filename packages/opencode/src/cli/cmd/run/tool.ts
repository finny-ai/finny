...
function runTask(p: ToolProps<typeof TaskTool>): ToolInline {
  const input = p.input as { tasks?: Array<{ subagent_type: string; description: string }>; subagent_type?: string; description?: string }
  if (input.tasks) {
    const count = input.tasks.length
    const icon = p.frame.status === "error" ? "✗" : p.frame.status === "running" ? "•" : "✓"
    return { icon, title: `Mandatory evidence batch (${count})`, description: "Parallel subagents" }
  }
  const kind = subagentLabel(input.subagent_type || "unknown")
...
function subagentLabel(value: string): string {
  return Locale.titlecase(value.replaceAll("_", " "))
}
...
function snapTask(p: ToolProps<typeof TaskTool>): ToolSnapshot {
  const input = p.input as { tasks?: Array<{ subagent_type: string; description: string }>; subagent_type?: string; description?: string }
  if (input.tasks) {
    const rows = input.tasks.map((task) => `${task.subagent_type}: ${task.description}`)
    return { kind: "task", title: "# Mandatory Evidence Batch", rows, tail: "" }
  }
  const kind = Locale.titlecase(input.subagent_type || "general")
  const desc = input.description
...
function scrollTaskStart(p: ToolProps<typeof TaskTool>): string {
  const input = p.input as {
    tasks?: Array<{ subagent_type?: string; description?: string }>
    subagent_type?: string
    description?: string
  }
  if (input.tasks) {
    const rows = input.tasks.map((task, index) => {
      const kind = subagentLabel(task.subagent_type || `subagent ${index + 1}`)
      return `- ${kind}: ${task.description || "mandatory evidence"}`
    })
    return [`# Mandatory Evidence Batch`, ...rows].join("\n")
  }
  const kind = subagentLabel(input.subagent_type || "general")
  const row = input.description || text(p.frame.state.title)
...
function taskResult(output: string): string | undefined {
  ...
  if (output.includes("<task_batch")) return output.trim()
...
function scrollTaskFinal(p: ToolProps<typeof TaskTool>): string {
  ...
  const input = p.input as { tasks?: Array<unknown>; subagent_type?: string; description?: string }
  if (input.tasks) {
    return `# Mandatory Evidence Batch\n${input.tasks.length} subagents`
  }
  const kind = subagentLabel(input.subagent_type || "general")
  const row = input.description || text(p.frame.state.title)
  if (!row) {
...
function permTask(p: ToolPermissionProps<typeof TaskTool>): ToolPermissionInfo {
  ...
  if (input.tasks) {
    ...
    return {
      ...
      lines: tasks.map((task) => `◉ ${task.subagent_type}: ${task.description}`),
    }
  }
...
export function toolEntryBody(commit: StreamCommit, raw: string): RunEntryBody | undefined {
  ...
  if (ctx.name === "task") {
    if (commit.phase === "start" && !("tasks" in ctx.input)) {
      return undefined
    }
