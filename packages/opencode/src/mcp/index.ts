...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      const cwd = mcp.cwd ? (path.isAbsolute(mcp.cwd) ? mcp.cwd : path.resolve(directory, mcp.cwd)) : directory
      ...
      return yield* Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          return {
            mcpClient,
            status: safeStatus,
            defs: McpRobinhood.isServer(key, mcp, managedRobinhood) ? McpRobinhood.filterTools(listed) : listed,
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("MCP.state")(function* () {
        ...
        const s: State = {
          config: managed.config ? { [McpRobinhood.SERVER_NAME]: managed.config } : {},
          status: {},
          clients: {},
          defs: {},
