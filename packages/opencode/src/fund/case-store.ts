...
export class FundCaseStoreError extends Error {
  constructor(
    readonly code:
      | "case_not_admitted"
      | "case_mismatch"
  ...
  for (const reference of references) {
    if (!admitted.has(evidenceKey(reference))) {
      throw new FundCaseStoreError(
        "evidence_mismatch",
        `Evidence ${reference.kind}:${reference.ref} is not bound to ${caseRecord.caseId}.`,
      )
    }
  }
}
...
function assertSnapshotKinds(input: FundProposalBody) {
  ...
  if (!advisoryWithoutSnapshots && input.portfolioSnapshot?.kind !== "portfolio_snapshot") {
    throw new FundCaseStoreError("evidence_mismatch", "A bound portfolio_snapshot is required for this action.")
  }
  if (!advisoryWithoutSnapshots && input.marketSnapshot?.kind !== "market_snapshot") {
    throw new FundCaseStoreError("evidence_mismatch", "A bound market_snapshot is required for this action.")
  }
  if (input.portfolioSnapshot && input.portfolioSnapshot.kind !== "portfolio_snapshot") {
    throw new FundCaseStoreError("evidence_mismatch", "portfolioSnapshot must have kind portfolio_snapshot.")
  }
  if (input.marketSnapshot && input.marketSnapshot.kind !== "market_snapshot") {
    throw new FundCaseStoreError("evidence_mismatch", "marketSnapshot must have kind market_snapshot.")
  }
}

async function run<A>(
  database: Database.Interface | undefined,
  effect: (database: Database.Interface) => Effect.Effect<A>,
): Promise<A> {
  if (database) return Effect.runPromise(effect(database))
  return Effect.runPromise(Effect.flatMap(Database.Service, effect).pipe(Effect.provide(Database.defaultLayer)))
}
...
function withStore<A>(database: Database.Interface, effect: Effect.Effect<A>) {
  return ensureAppendOnly(database).pipe(Effect.andThen(effect), Effect.orDie)
}

export namespace FundCaseStore {
            ...
            Effect.gen(function* () {
              ...
              if (existing) {
                ...
                if (
                  ...
                  throw new FundCaseStoreError(
                    "duplicate_case",
                    "The trigger message is already bound to a different immutable fund event.",
                  )
                }
                return current
              }
              ...
              if (manager?.agent !== FUND_MANAGER_AGENT) {
                throw new FundCaseStoreError(
                  "invalid_lineage",
                  "Fund cases may be admitted only for a canonical Fund Manager session.",
                )
              }
    ...
    return run(database, (service) =>
      withStore(
        ...
        service.db
          ...
          .get()
          .pipe(Effect.map((row) => (row ? caseFromRow(row) : undefined)), Effect.orDie),
      ),
    )
  }
      ...
      withStore(
        ...
        Effect.gen(function* () {
          const children = yield* service.db
            ...
            .where(
              and(
                eq(FundCaseChildTable.case_id, caseRecord.caseId),
                eq(FundCaseChildTable.role, input.agent),
              ),
            )
            .orderBy(asc(FundCaseChildTable.attempt_no))
            .all()
          const currentDraft = FUND_DRAFT_REVIEWERS.includes(
            input.agent as (typeof FUND_DRAFT_REVIEWERS)[number],
          )
            ? yield* service.db
          ...
          for (const child of children) {
            ...
            if (task?.status === TaskState.Status.completed) {
              const report = yield* service.db
                ...
                .get()
              const staleMaterialReview =
                currentDraft && report && report.draftSha256 !== currentDraft.draftSha256
  ...
  export async function admitChildAttempt(
    ...
    if (!launch.allowed) throw new FundCaseStoreError(launch.code, launch.message)
    return run(database, (service) =>
      withStore(
        service,
        service.db
            ...
            Effect.gen(function* () {
              ...
              if (existing) {
                if (
                  ...
                  throw new FundCaseStoreError(
                    "invalid_lineage",
                    "The child session is already bound to another case, parent, or specialist role.",
                  )
                }
                return {
              ...
              if (
                ...
                throw new FundCaseStoreError(
                  "invalid_lineage",
                  "A live canonical TaskState row with matching parent and role is required.",
                )
              }
              const total = yield* tx
                ...
                .where(
                  and(
                    eq(FundCaseChildTable.case_id, launch.caseRecord.caseId),
                    eq(FundCaseChildTable.role, input.agent),
                  ),
                )
                .get()
              ...
              if (attemptNo > 2) {
                throw new FundCaseStoreError(
                  "attempt_budget_exhausted",
                  `${input.agent} exhausted the two-attempt budget for ${launch.caseRecord.caseId}.`,
                )
              }
              yield* tx
      ...
      withStore(
        ...
        Effect.gen(function* () {
          const child = yield* service.db
            ...
            .get()
          if (!child) return
          const row = yield* service.db
            .select()
            .from(FundCaseTable)
            .where(eq(FundCaseTable.case_id, child.case_id))
            .get()
          if (!row) return
          ...
          return [
            ...
            stable({
              ...
              ...(child.draft_sha256
                ? { materialDraftSha256: child.draft_sha256, materialDraft: draft?.payload }
                : {}),
            }),
            "</fund-case-contract>",
          ].join("\n")
          ...
          .transaction((tx) =>
            Effect.gen(function* () {
              ...
              if (
                ...
                throw new FundCaseStoreError(
                  "invalid_lineage",
                  "Report submission requires the matching running specialist child TaskState.",
                )
              }
              ...
              if (prior) {
                throw new FundCaseStoreError(
                  "report_already_submitted",
                  "Each admitted specialist child may submit exactly one report.",
                )
              }
              const caseRow = yield* tx
                .select()
                .from(FundCaseTable)
                .where(eq(FundCaseTable.case_id, child.case_id))
                .get()
              if (!caseRow) throw new FundCaseStoreError("case_not_admitted", "The child case no longer exists.")
              const caseRecord = caseFromRow(caseRow)
              const evidence = canonicalEvidence(input.report.evidence)
              assertEvidence(caseRecord, evidence)
            ...
            Effect.gen(function* () {
              ...
              for (const reference of references) {
                ...
                if (!report || report.role !== reference.agent || !isFundSpecialistAgent(reference.agent)) {
                  throw new FundCaseStoreError(
                    "report_missing",
                    `The referenced ${reference.agent} report was not submitted for ${caseRecord.caseId}.`,
                  )
                }
                const task = yield* tx
                  .select()
                ...
                if (
                  ...
                  throw new FundCaseStoreError(
                    "report_not_completed",
                    `${reference.agent} report cannot be admitted before its exact child TaskState completes.`,
                  )
                }
  ...
  function validateRequiredReports(
    ...
    for (const role of required) {
      if (!admitted.has(role)) {
        throw new FundCaseStoreError(
          "required_specialist_missing",
          `${role} is required before ${phase === "draft" ? "recording the material draft" : "finalizing the proposal"}.`,
        )
      }
    }
  }
  ...
  export async function createDraft(
    ...
    const caseRecord = await caseForManagerTurn(input.managerSessionID, input.triggerMessageID, database)
    if (!caseRecord) throw new FundCaseStoreError("case_not_admitted", "No immutable fund case is active.")
    const policy = FUND_ACTION_POLICY[input.body.action]
    if (policy.riskTier !== "material_change") {
      throw new FundCaseStoreError("draft_mismatch", "Only material-change actions use a pre-review draft.")
    }
            ...
            Effect.gen(function* () {
              ...
              if (existing) {
                if (existing.draft_sha256 !== draftSha256) {
                  throw new FundCaseStoreError(
                    "draft_mismatch",
                    "A different immutable material draft is already bound to this case.",
                  )
                }
                return {
  ...
  export async function finalizeProposal(
    ...
    if (policy.riskTier === "material_change") {
      ...
      if (reviewerReports.some((report) => report.draftSha256 !== draft!.draft_sha256)) {
        throw new FundCaseStoreError(
          "draft_mismatch",
          "Validator and sentinel attestations must be bound to the exact material draft digest.",
        )
      }
      ...
      throw new FundCaseStoreError("draft_mismatch", "Non-material proposals cannot attach a material draft.")
    }
            ...
            Effect.gen(function* () {
              ...
              if (existing) {
                if (existing.proposal_sha256 !== proposalSha256) {
                  throw new FundCaseStoreError(
                    "proposal_already_finalized",
                    "This case and event already has a different immutable final proposal.",
                  )
                }
    ...
    return run(database, (service) =>
      withStore(
        ...
        service.db
          ...
          .get()
          .pipe(Effect.map((row) => (row ? (JSON.parse(row.payload) as FundActionProposal) : undefined)), Effect.orDie),
      ),
    )
  }
}
