...
const OPENAI_SCHEMA_SANITIZER_PACKAGES = new Set(["@ai-sdk/openai", "@ai-sdk/azure"])

function isPlainObject(value: unknown): value is JsonRecord {
  return typeof value === "object" && value !== null && !Array.isArray(value)
}

function sanitizeSchemaCollection(value: unknown, sanitize: (value: unknown) => unknown): unknown {
  if (!isPlainObject(value)) return undefined
  return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, sanitize(item)]))
}

function sanitizeJsonSchemaArray(value: unknown, sanitize: (value: unknown) => unknown): unknown[] | undefined {
  if (!Array.isArray(value)) return undefined
  return value.map(sanitize)
}

function copyOpenAISchemaKeywords(value: JsonRecord): JsonRecord {
...
function addOpenAIObjectKeywords(value: JsonRecord, result: JsonRecord) {
  const properties = sanitizeSchemaCollection(value.properties, sanitizeOpenAISchema)
  ...
  for (const key of COMPOSITION_KEYS) {
    const sanitized = sanitizeJsonSchemaArray(value[key], sanitizeOpenAISchema)
    if (sanitized) result[key] = sanitized
  }
}

function addOpenAIDefinitions(value: JsonRecord, result: JsonRecord) {
  for (const key of ["$defs", "definitions"]) {
    const sanitized = sanitizeSchemaCollection(value[key], sanitizeOpenAISchema)
    if (sanitized) result[key] = sanitized
  }
}

function openAISchemaTypes(value: JsonRecord): string[] {
  if (typeof value.type === "string") return JSON_SCHEMA_TYPES.includes(value.type as any) ? [value.type] : []
  if (!Array.isArray(value.type)) return []
  return value.type.filter((item) => typeof item === "string" && JSON_SCHEMA_TYPES.includes(item as any))
}

function hasOpenAIComposition(result: JsonRecord): boolean {
  return COMPOSITION_KEYS.some((key) => key in result)
}

function inferOpenAISchemaTypes(value: JsonRecord, result: JsonRecord, schemaTypes: string[]): string[] {
  ...
  if (["properties", "required", "additionalProperties"].some((key) => key in value)) return ["object"]
...
function applyOpenAISchemaTypes(result: JsonRecord, inferredTypes: string[]): JsonRecord {
...
// Mirrors upstream OpenCode commit 213ff3f2d77fabfcfe3148bc21d7aa607e633737.
function sanitizeOpenAISchema(value: unknown): unknown {
  ...
  const result = copyOpenAISchemaKeywords(value)
  addOpenAIObjectKeywords(value, result)
  addOpenAIDefinitions(value, result)

  const schemaTypes = openAISchemaTypes(value)
  if (schemaTypes.length === 0 && (typeof result.$ref === "string" || hasOpenAIComposition(result))) return result

  return applyOpenAISchemaTypes(result, inferOpenAISchemaTypes(value, result, schemaTypes))
}

function moveMoonshotTypeIntoCombiners(result: JsonRecord) {
  if (typeof result.type !== "string") return

  for (const key of ["anyOf", "oneOf"]) {
    const options = result[key]
    if (!Array.isArray(options)) continue
    result[key] = options.map((item) =>
      isPlainObject(item) && item.type === undefined && item.$ref === undefined ? { type: result.type, ...item } : item,
    )
    delete result.type
  }
}

function sanitizeMoonshotObject(value: JsonRecord): JsonRecord {
  ...
  // Moonshot rejects combiner schemas with `type` on the parent; each branch needs its own type.
  moveMoonshotTypeIntoCombiners(result)
  return result
}
...
function sanitizeMoonshotSchema(value: unknown): unknown {
  ...
  if (!isPlainObject(value)) return value
  return sanitizeMoonshotObject(value)
}

function shouldSanitizeOpenAISchema(model: Provider.Model): boolean {
  return OPENAI_SCHEMA_SANITIZER_PACKAGES.has(model.api.npm)
}

function sanitizeOpenAIModelSchema(model: Provider.Model, value: JSONSchema7): JSONSchema7 {
  if (!shouldSanitizeOpenAISchema(model)) return value
  // Codex also applies lossy compaction above 4 KB; defer that until OpenCode needs the same schema budget.
  return sanitizeOpenAISchema(value) as JSONSchema7
}

function shouldSanitizeMoonshotSchema(model: Provider.Model): boolean {
  return [model.providerID === "moonshotai", model.api.id.toLowerCase().includes("kimi")].some(Boolean)
}

function sanitizeMoonshotModelSchema(model: Provider.Model, value: JSONSchema7): JSONSchema7 {
  if (!shouldSanitizeMoonshotSchema(model)) return value
  const sanitized = sanitizeMoonshotSchema(value)
...
export function schema(model: Provider.Model, schema: JSONSchema7): JSONSchema7 {
  ...
  */
  ...
  schema = sanitizeOpenAIModelSchema(model, schema)
  schema = sanitizeMoonshotModelSchema(model, schema)
  ...
  if (model.providerID === "google" || model.api.id.includes("gemini")) {
    ...
    const sanitizeGemini = (obj: any): any => {
      ...
      if (Array.isArray(result.type)) {
        const types = [...new Set(result.type.filter((type: unknown): type is string => typeof type === "string"))]
