...
class AlpacaBroker(Broker):
    ...
    def __init__(self, key_id: Optional[str] = None, secret: Optional[str] = None,
                 oauth_token: Optional[str] = None, paper: bool = True,
        ...
        has_key_pair = bool(key_id and secret)
        has_partial_key_pair = bool(key_id or secret) and not has_key_pair
        if has_partial_key_pair or bool(oauth_token) == has_key_pair:
            raise RuntimeError("Configure exactly one Alpaca auth method: OAuth token or API key pair")
        if oauth_token and (not paper or (endpoint and endpoint.rstrip("/") != "https://paper-api.alpaca.markets")):
            raise RuntimeError("Alpaca OAuth credentials are restricted to the canonical paper endpoint")

        trading_kwargs = ({"oauth_token": oauth_token, "paper": paper} if oauth_token else
                          {"api_key": key_id, "secret_key": secret, "paper": paper})
        if endpoint:
        ...
        try:
            ...
            raise
        self._stock_data = (StockHistoricalDataClient(oauth_token=oauth_token) if oauth_token else
                            StockHistoricalDataClient(api_key=key_id, secret_key=secret))
