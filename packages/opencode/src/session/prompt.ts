...
import { runSessionPreflight } from "./finny-preflight-runner"
import { LLM } from "./llm"
  ...
  Effect.gen(function* () {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      yield* runSessionPreflight({
        ...
        parts: message.parts,
        setStatus: (next) => status.set(input.sessionID, next),
        onError: (error) =>
          Effect.gen(function* () {
            yield* events.publish(Session.Event.Error, {
              sessionID: input.sessionID,
...
let runtime: ReturnType<typeof makeRuntime<typeof Service, Interface, never>> | undefined
function getRuntime() {
  return (runtime ??= makeRuntime(Service, defaultLayer as Layer.Layer<Service>))
}

export function cancel(sessionID: SessionID) {
  return getRuntime().runPromise((prompt) => prompt.cancel(sessionID))
}
...
export { createStructuredOutputTool } from "./structured-output-tool"
const bashRegex = /!`([^`]+)`/g
// Match [Image N] as single token, quoted strings, or non-space sequences
const argsRegex = /(?:\[Image\s+\d+\]|"[^"]*"|'[^']*'|[^\s"']+)/gi
const placeholderRegex = /\$(\d+)/g
const quoteTrimRegex = /^["']|["']$/g
const lazyNode = <T extends LayerNode.Node<unknown, unknown>>(get: () => T) =>
  LayerNode.make(
    Layer.suspend(() => LayerNode.buildLayer(get())),
    [],
  ) as T
