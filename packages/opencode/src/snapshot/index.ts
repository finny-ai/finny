    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Snapshot.state")(function* (ctx) {
        ...
        const seed = Effect.fnUntraced(function* () {
          ...
          if ((yield* exists(sourceIndex)) && (yield* sourceRoot()) === state.worktree) {
        ...
        const sourceRoot = Effect.fnUntraced(function* () {
          const result = yield* git(["rev-parse", "--path-format=absolute", "--show-toplevel"], {
            cwd: state.directory,
          })
          if (result.code !== 0) return
        ...
        const normalizeCandidatePaths = Effect.fnUntraced(function* (files: string[]) {
          if (!files.length) return files
          const root = yield* sourceRoot()
          if (!root) return files

          const rel = path.relative(root, state.directory).replaceAll("\\", "/")
          if (!rel || rel.startsWith("..")) return files
        ...
        const add = Effect.fnUntraced(function* () {
          ...
          const tracked = yield* normalizeCandidatePaths(diff.text.split("\0").filter(Boolean))
          const untracked = yield* normalizeCandidatePaths(other.text.split("\0").filter(Boolean))
