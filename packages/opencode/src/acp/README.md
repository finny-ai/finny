...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for Finny.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal Finny sessions
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 finny acp
```
...
- **Terminal Integration**: Full terminal support via Finny's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | finny acp
```
...
ACP sessions map cleanly to Finny's internal session model:
