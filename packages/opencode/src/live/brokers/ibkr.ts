...
export const ibkrSpec: BrokerSpec = {
  ...
  promptFragment: [
    ...
    "",
    "**Research/backtesting versus live execution:**",
    "- Brokerage eligibility limits live deployment only. They must never block market research, exact-ticker data extraction, strategy generation, validation, or backtesting.",
    "- Preserve exact regional listings throughout research: India `.NS`/`.BO`; Canada `.TO`/`.V`; Europe `.AS`/`.BR`/`.DE`/`.L`/`.MC`/`.MI`/`.PA`/`.SW`; China and Hong Kong `.SS`/`.SZ`/`.HK`.",
    "- Never replace a regional listing with a US ADR, US ETF, country ETF, or other proxy unless the user explicitly requests that proxy.",
    "- If the exact regional listing is not executable through the active IBKR runtime, complete the research/backtest and mark only live deployment unavailable. Recommend connecting the matching regional brokerage rather than changing the symbol.",
    "",
    "**Live-execution asset classes:**",
    ...
    "- Live deployment unavailable in this runtime: FX spot, mutual funds, and non-US equities/options/futures. This restriction does not apply to research or backtesting.",
    "",
    "**`config.symbol` format (one string per instrument):**",
    "- Equity: bare ticker — `\"AAPL\"`, `\"SPY\"`, `\"TSLA\"`.",
    "- Regional equity research/backtest: preserve the exact exchange suffix — `\"0700.HK\"`, `\"600519.SS\"`, `\"300750.SZ\"`, `\"RELIANCE.NS\"`, `\"SHOP.TO\"`.",
