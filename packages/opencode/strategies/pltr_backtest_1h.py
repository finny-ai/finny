...
class Strategy:
    ...
    def on_tick(self, bar: dict) -> str:
        ...
        # 4. Entry Logic
            ...
            # Assuming 1 week expiry. On 1h charts, 1 week is approx 35 bars.
            if bars_since_open > 35: 
                self.post_earnings_open = None
                return "HOLD"
...
def run_backtest():
    print("Fetching data...")
    # Fetch 1h data for PLTR for last 2 years (approx 730 days)
    ticker = "PLTR"
    try:
        data = yf.download(ticker, interval="1h", period="2y", progress=False)
