...
def resolve_execution_profile(asset_class: str, execution_cfg: Dict[str, Any]) -> Dict[str, Any]:
    requested = str(execution_cfg.get("profile_id") or execution_cfg.get("execution_profile") or default_profile_id(asset_class))
    ...
    for key, value in execution_cfg.items():
        if key in {"profile_id", "execution_profile", "scenario", "scenarios"}:
            continue
    ...
    return {
        ...
        "scenarios": {
            ...
            "base": dict(effective),
            "stressed": {**effective, **profile.stress.get("atr_stress", {}), **next(
