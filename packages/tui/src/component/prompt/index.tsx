...
export function Prompt(props: PromptProps) {
  ...
  async function submitInner() {
    ...
    if (store.mode !== "shell" && sessionID) {
      sync.set("preflight_steps", sessionID, [])
    }
    ...
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          <Switch>
            <Match when={status().type !== "idle" && status().type !== "preflight"}>
              <box
                flexDirection="row"
                gap={1}
                flexGrow={1}
          ...
          <Show when={status().type !== "retry" && props.showShortcutHints !== false}>
