...
import { PreflightProgressPanel, usePreflightSteps } from "./preflight-progress"
...
export function Session() {
  ...
  const lastUserMessageID = createMemo(() => messages().findLast((x) => x.role === "user")?.id)
  ...
  const preflight = usePreflightSteps(route.sessionID, toBottom)

  const showPreflightForLastUser = createMemo(() => {
    const id = lastUserMessageID()
    if (!id) return false
    if (preflight.steps().length > 0 || preflight.active()) return true

    const msg = messages().findLast((m) => m.role === "user")
    if (!msg || msg.id !== id) return false
    if (messages().at(-1)?.role !== "user") return false

    const status = sync.data.session_status[route.sessionID]
    return status?.type === "preflight"
  })

  const preflightPending = createMemo(() => false)
                    ...
                    <Switch>
                      ...
                      <Match when={message.role === "user"}>
                        <UserMessage
                          ...
                          pending={pending()}
                          preflightAttached={showPreflightForLastUser() && message.id === lastUserMessageID()}
                        >
                          <Show when={showPreflightForLastUser() && message.id === lastUserMessageID()}>
                            <PreflightProgressPanel
                              steps={preflight.steps()}
                              active={preflight.active() || preflightPending()}
                              attached
                            />
                          </Show>
                        </UserMessage>
      ...
      <Show when={text()}>
        ...
        >
          <box
            ...
            paddingTop={1}
            paddingBottom={props.children ? 0 : 1}
            paddingLeft={2}
        ...
        <Match when={true}>
          <Show
            fallback={
              ...
              >
                ~ {props.failed && props.failure ? props.failure : props.pending}
              </text>
            }
            when={props.complete}
          >
