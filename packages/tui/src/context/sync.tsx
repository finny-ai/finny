...
import { preflightStepsFromStatus, type PreflightStep } from "../routes/session/preflight-steps"
...
export const {
  ...
  init: () => {
    ...
    const [store, setStore] = createStore<{
      ...
      preflight_steps: {
        [sessionID: string]: PreflightStep[]
      }
      session_diff: {
    ...
    function markPreflightStepsDone(sessionID: string) {
      setStore(
        "preflight_steps",
        produce((draft) => {
          const steps = draft[sessionID]
          if (!steps?.some((step) => step.status === "active")) return
          draft[sessionID] = steps.map((step) =>
    ...
    function applyPreflightSteps(sessionID: string, status: SessionStatus) {
      if (status.type === "preflight") {
        setStore("preflight_steps", sessionID, preflightStepsFromStatus(status))
        return
      }
      if (status.type === "busy") markPreflightStepsDone(sessionID)
    }

    function seedPreflightSteps(statuses: Record<string, SessionStatus>) {
      setStore(
        "preflight_steps",
        produce((draft) => {
          for (const [sessionID, status] of Object.entries(statuses)) {
            if (status.type === "preflight") draft[sessionID] = preflightStepsFromStatus(status)
          }
        }),
      )
    }
    ...
    event.subscribe((event, { workspace }) => {
      switch (event.type) {
        ...
        case "session.status": {
          ...
          applyPreflightSteps(sessionID, status)
          break
        }
