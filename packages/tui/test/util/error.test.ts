...
describe("util.error", () => {
  ...
  test("extracts message from SDK error response bodies", () => {
    const err = { data: { message: "Finny Home is controlled by FINNY_HOME" } }
    expect(errorMessage(err)).toBe("Finny Home is controlled by FINNY_HOME")

    const data = errorData(err)
    expect(data.message).toBe("Finny Home is controlled by FINNY_HOME")
  })
