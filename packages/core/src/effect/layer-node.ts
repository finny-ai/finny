...
export function buildLayer<A, E>(node: Node<A, E>, options?: { readonly replacements?: readonly Replacement[] }) {
  ...
  const visit = (input: AnyNode): RuntimeLayer => {
    if (!input) {
      throw new Error("LayerNode.buildLayer received an undefined dependency")
    }
    ...
    try {
      const dependencies = node.dependencies.map((dependency, index) => {
        if (!dependency) {
          throw new Error(
            `LayerNode.buildLayer received an undefined dependency at ${node.kind}#${ids.get(node)}[${index}]`,
          )
        }
