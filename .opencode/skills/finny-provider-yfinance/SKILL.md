...
Write exactly one OHLCV CSV under `allowed_data_dir` only when rows are usable for the requested source attempt. After verification and optional analysis, call `finny_dataset_evidence_finalize` once with the relative CSV path, exact Yahoo feed/venue/provider symbol, and the `auto_adjust` split/dividend treatment used by the fetch. Never write or patch the manifest yourself; the runtime owns DatasetEvidenceV2 identity, calendar reconciliation, hashes, quality, qualification, and lineage.
