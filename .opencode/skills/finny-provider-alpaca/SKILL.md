...
Record `source: "alpaca"`, requested and actual coverage, source attempts, `run_id`, quality counts, exact `output_path`, and `usable_for_parent`. Treat authentication, subscription/feed, empty-result, and pagination failures as provider failures; do not label them as yfinance retrieval failures.
